...
//    @AfterReturning("ptSelect()")
    @AfterReturning(pointcut = "execution(* com.itheima.dao.BookDao.select())", returning = "result1")
    //因为是在return后执行的，所以要注意方法要传入上个return的对象
    public void methodSelect2(Object result1){
        System.out.println("这是在select方法正常运行后执行,才会执行，返回结果是"+result1);
    }
